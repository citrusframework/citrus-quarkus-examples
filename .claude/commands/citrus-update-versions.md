...
3. **Compare versions.** For each version property found in the Citrus release POM, check whether the same property name (or a clearly equivalent one) exists in the local POM files. Classify each match into one of:
   - **Preserved** — the local version should not be changed due to known issues with the sample code. These will NOT be updated but will be reported. Currently, no libraries are classified as preserved.
...
7. **Verify with a full Maven build.** Run `mvn verify` from the repository root to verify that all version changes are compatible. If the build fails:
   ...
   - Re-run `mvn verify` and repeat until the build succeeds.
